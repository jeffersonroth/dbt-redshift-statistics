...
while read mock_id; do
    ...
    DBT_ENVIRONMENT=dev dbt run --target test --models monitoring --vars "mock_id: $mock_id"
    DBT_ENVIRONMENT=dev dbt snapshot --target test -s table_info_scd --vars "mock_id: $mock_id"
    if [ $? -ne 0 ]; then
